...
JOBBER_NOTIFY_ERR=false
JOBBER_NOTIFY_FAIL=false

if [ -n "${VOLUMERIZE_JOBBER_TIME}" ]; then
  JOBBER_CRON_SCHEDULE=${VOLUMERIZE_JOBBER_TIME}
fi

if [ -n "${VOLUMERIZE_JOBBER_NOTIFY_ERR}" ]; then
  JOBBER_NOTIFY_ERR=${VOLUMERIZE_JOBBER_NOTIFY_ERR}
fi

if [ -n "${VOLUMERIZE_JOBBER_NOTIFY_FAIL}" ]; then
  JOBBER_NOTIFY_FAIL=${VOLUMERIZE_JOBBER_NOTIFY_FAIL}
fi
...
JOB_NOTIFY_ERR1=$JOBBER_NOTIFY_ERR
JOB_NOTIFY_FAIL1=$JOBBER_NOTIFY_FAIL
